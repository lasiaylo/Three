	...
	public class YarnCharacter : MonoBehaviour {
			...
			get {
				if (!offsetUsesRotation) {
					...
					return transform.position +
					       transform.TransformPoint(messageBubbleOffset); // convert offset into local space
				}
			}
		}
		...
		void Start() {
			if (YarnCharacterView.instance == null) {
				Debug.LogError(
					"YarnCharacter can't find the YarnCharacterView instance! Is the 3D Dialogue prefab and YarnCharacterView script in the scene?");
				return;
			}
